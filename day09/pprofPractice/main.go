...
// 那么最好的办法是在应用退出的时候把 profiling 的报告保存到文件中，进行分析
...
// 不管哪种方式，你的 HTTP 服务都会多出/debug/pprof endpoint，访问127.0.0.1:8080/debug/pprof
// 这个路径下还有几个子页面：
...
// go tool pprof命令
// 不管是工具型应用还是服务型应用，我们使用相应的pprof库获取数据之后
// 下一步的都要对这些数据进行分析，我们可以使用go tool pprof命令行工具。
// go tool pprof最简单的使用方式为:
// go tool pprof [binary] [source]
// binary 是应用的二进制文件，用来解析各种符号；
// source 表示 profile 数据的来源，可以是本地的文件，也可以是 http 地址。
// 注意事项：获取的 Profiling 数据是动态的，要想获得有效的数据，请保证应用处于较大的负载
// （比如正在生成中运行的服务，或者通过其他工具模拟访问压力）。否则如果应用处于空闲状态，得到的结果可能没有任何意义
...
// go tool pprof cpu.pprof 命令行交互式查看性能分析结果
// 在交互界面输入top3来查看程序中占用CPU前3位的函数
// 还可以使用list 函数名命令查看具体的函数分析，如list logicCode
// 通过分析发现大部分CPU资源被17行占用，我们分析出select语句中的default没有内容会导致上面的case v:=<-c:一直执行
...
// 方框之间的线条代表函数之间的调用关系
// 线条上的数字表示函数调用的次数
// 方框中的第一行数字表示当前函数占用CPU的百分比
// 第二行数字表示当前函数累计占用CPU的百分比


// go-torch和火焰图
// 火焰图（Flame Graph）是 Bredan Gregg 创建的一种性能分析图表
// 因为它的样子近似 🔥而得名。上面的 profiling 结果也转换成火焰图
// 如果对火焰图比较了解可以手动来操作，不过这里我们要介绍一个工具：go-torch
// 这是 uber 开源的一个工具，可以直接读取 golang profiling 数据，并生成一个火焰图的 svg 文件。

// 安装go-touch
// go get -v github.com/uber/go-torch
// 火焰图 svg 文件可以通过浏览器打开
// 它对于调用图的最优点是它是动态的：可以通过点击每个方块来 zoom in 分析它上面的内容。

// 火焰图的调用顺序从下到上，每个方块代表一个函数，它上面一层表示这个函数会调用哪些函数，方块的大小代表了占用 CPU 使用的长短
// 火焰图的配色并没有特殊的意义，默认的红、黄配色是为了更像火焰而已。

// go-torch工具的使用非常简单，没有任何参数的话，它会尝试从http://localhost:8080/debug/pprof/profile获取 profiling 数据
// 它有三个常用的参数可以调整：
...
// Windows平台，需要把go-torch/render/flamegraph.go文件中的GenerateFlameGraph按如下方式修改，然后在go-torch目录下执行go install即可。
