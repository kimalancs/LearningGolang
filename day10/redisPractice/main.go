...
// Redis与Memcached比较
// Memcached中的值只支持简单的字符串，Reids支持更丰富的5种数据结构类型
// Redis的性能比Memcached好很多
// Redis支持RDB持久化和AOF持久化
// Redis支持master/slave模式。

// Go语言中使用第三方库https://github.com/go-redis/redis连接Redis数据库并进行操作
// 使用以下命令下载并安装:
...
func main() {
	err := initClient()
	if err != nil {
		fmt.Printf("init client failed, err:%v", err)
	}
